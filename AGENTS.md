...
- 桌面标题栏左侧仅保留搜索入口，Ctrl+K（兼容 Cmd+K）打开搜索弹层，侧栏不再显示单词搜索；标题栏中间是词书菜单，当前六级可用、四级只显示“即将上线”。Windows 保留原生窗口按钮和拖动区域。手机搜索位于顶栏左侧，底部导航保留四项。
- 侧栏底部只显示头像与用户名（当前使用邮箱前缀），点击展开账号状态和退出入口；上方提供“更新日志”，默认展示最新正式发布，支持历史版本。摘要维护在 `src/release-notes.ts`，发布时与 `docs/CHANGES.md` 同步，不把待发布改动写成已发布内容。
- 登录令牌到期或接口返回 401 后显示“未登录”，点击才重新登录；保留原账号本机学习与进度，不清除受保护离线会话。重新登录按账号读取进度，网络故障本身不等于登录失效。
...
- 记忆增强独立保存在 `books/<code>/enhancements/`，由词书清单指定；不得覆盖原文或未经逐词复核自动续签审核哈希。音形分块不当作词根拆解，标题各块保持相同字体与字重。应用和官网示例均默认不分块，点击切换，发音加载/播放时临时分块，结束/停止/失败恢复原状态，切词重置。
...
- 首页使用实际当前学习日，其他日期只作为明确标记的预览。每日学习连续进行，评级后直接进入下一待学词，不分阶段、不弹出中途休息页；原词根组、依赖顺序和整日完成规则保留。
...
- Windows 和安卓安装包预装完整词书、全部发音、原配图和音标字体，首次联网登录后直接读取包内资源离线学习，不二次下载或复制到 IndexedDB。开发预览默认直接读取 `data/`，按需复用本机 `book` 目录或 `release/` 完整 APK 的媒体，不全量下载或解包；只有显式 `VITE_CYWORD_BOOK_DOWNLOAD=1` 才恢复旧 IndexedDB 下载流程。进度默认仅保存本机，按设备与账号只读合并导入旧云端进度一次，落盘成功后才标记完成；未完成导入不能阻断本地学习，401 不清除离线会话。
- 云端代码、配置、D1 表与旧记录保留；显式设置 `VITE_CYWORD_PROGRESS_MODE=cloud` 可恢复同步。该模式通过认证后的 `/api/progress` 和 D1 `learning_progress` 表；写入检查修订号，409 合并后重试。旧版收藏删除标记继续保留，禁止整份旧进度覆盖更新。详见 `docs/OFFLINE.md`。
...
- GitHub 仓库 `https://github.com/cheng-yi-cc/CYword` 已公开，可提供源码和 Release 入口；已安装应用的下载与自动更新继续使用官网，不能因公开仓库而更换现有更新源。内容来源与授权证据状态记录在 `docs/CONTENT-SOURCES.md`，不得把软件许可证或来源记录等同于第三方内容的授权证明。
...
- Windows 差量更新沿用 electron-updater；官网只对缺失的版本化旧 blockmap 地址按版本唯一定位并临时跳转，保留不可变资产。安卓差量清单为 APK 同目录的 `.apk.blocks.json`，按 ZIP 数据边界及 SHA-256 复用已安装 APK；校验整包、应用身份、递增版本及相同签名后才允许系统安装。发布先确认官网 `X-CYword-Android-Differential` 能力，先上传 APK 和分块清单，最后更新独立指针。
- 安卓检查更新只读取官网独立安卓清单，与安装应用的版本号比较；启动及回到前台自动检查（间隔 6 小时），“我的”保留手动检查与失败重试。下载、系统安装均由用户点击，差量失败保留已校验分块，不自动改为全量下载；完整 APK 作为明确入口，由浏览器打开经校验的官网版本化地址。不影响学习进度或 Windows 更新源。
...
- 本地预览：`npm run dev`
- 浏览器预览：`npm run dev:web`；手机预览：`npm run dev:mobile`（5173，直接使用本机词书和媒体，默认模拟登录和旧进度导入）；`CYWORD_REAL_AUTH=1` 切换生产认证和旧进度导入，词书仍默认本地直读；联调旧线上词书下载需同时设 `VITE_CYWORD_BOOK_DOWNLOAD=1`
...
- 官网交互测试：`npm run test:site:ui`（桌面和手机示例）
...
- [本地词书、一次性导入与云端恢复](docs/OFFLINE.md)
