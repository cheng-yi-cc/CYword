...
- Windows 和安卓安装包预装完整词书、全部发音、原配图和音标字体，首次联网登录后直接读取包内资源离线学习，不二次下载或复制到 IndexedDB；浏览器开发预览保留下载至 IndexedDB 的流程。进度默认仅保存本机，按设备与账号只读合并导入旧云端进度一次，落盘成功后才标记完成；未完成导入不能阻断本地学习，401 不清除离线会话。
...
- 本地预览：`npm run dev`
- 浏览器预览：`npm run dev:web`；手机预览：`npm run dev:mobile`（5173，默认本地编译词书、模拟登录和旧进度导入）；`CYWORD_REAL_AUTH=1` 同时切换生产认证、旧进度导入和词书，未发布数据可在默认本地模式预览
