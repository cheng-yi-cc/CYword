...
CYword 是一款支持 Windows 和安卓的背单词应用。它先按真正词根组织单词，再展示音标、翻译、巧记、构词分析和例句。词书在学习时联网加载，学习记录按账号保存在设备上。0.4.4 已实现手机与电脑的进度同步代码，正式云同步尚待 D1 授权、建表和接口部署；截至 2026-09-20，安装包仅完成本地交付，未发布到官网。
...
云端同步上线前，必须完成 [安卓与同步说明](docs/ANDROID.md) 中的 D1 建表和官网接口部署；仅安装客户端不会自动创建线上接口。
