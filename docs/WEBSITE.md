...
- 自 0.3.0 起，安装包不内置六级详情，运行时通过词书接口加载 5166 个唯一单词。没有手机版、Mac 版、四级或考研词书。
...
生产环境已绑定私有桶 `cyword-book-data`，当前数据版本为 `353a5bffec631ba5`。目录接口返回 5166 词；普通学习日以一次 POST 返回 179 个唯一单词、JSON 正文 2899177 字节。最坏累计复习测试以一次 POST 返回全部 5166 个单词、JSON 正文 79332804 字节，用时约 12.7 秒，证明分片读取和流式响应没有触发 Worker 内存失败。无 `Content-Length` 的超限请求返回 413。
