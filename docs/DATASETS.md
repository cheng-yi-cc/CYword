...
当前版本一次构建一本词书，默认 `cet6`。顶栏词书菜单仅展示当前六级和不可选择的四级预告。真正启用多词书切换前，须将运行时目录按词书代码分层并完成各书进度隔离，再启用菜单选项；不要把多本词书合并到一组 CSV。
...
原巧记内容及引用不得修改，也不得用运行时替换绕过此约定。唯一展示例外：单词巧记页面可隐藏行首“针对第 N 个元素采用词根词缀分析：”，不补序号，不修改源数据、其后的正文与引用或词根词缀巧记。

排课区分必须先学的熟词与已解释词根含义的回指；后者保留悬浮窗补充。互相依赖的词根组同日安排，必要时在组间穿插基础词；生成的 `exposureOrder` 指定当天安排顺序。重排不增删词条、学习组或安排次数。运行时任何已有评级的词均计入相关学习日的已学进度，实际评级曝光另行记录，详见[进度模型](ARCHITECTURE.md)。
