...
四级当前仍为编辑材料，未创建正式完整 `books/cet4/`。采用新方案的选词审计、共用构词与 100 词应用试验见 [编辑入口](../authoring/cet4/README.md)。共用登记权威源为 `authoring/shared/morphemes.csv`，四级旧登记文件由脚本投影；不得从六级的相同字母直接推定相同构词身份。
...
四级已完成阶段 1、2，阶段 3—5 编写中：`authoring/cet4/wordlist.csv` 冻结 6,161 个学习词条；`sample/` 保存 30 词完整样板及 19 张规范 CSV，`content/` 新增 105 词原创初稿。共 135 词有原创巧记，仍有 6,026 词待写；新增初稿尚未完成构词登记、逐段释义及二次核实，不能计作已复核正式词条。`references/` 保存 ECDICT 参考和许可，匹配结果不计入原创完成。来源、规范和逐词进度见 [四级编辑入口](../authoring/cet4/README.md)。当前仍没有正式 `books/cet4/`。用 `npm run cet4:review` 可在 5188 端口逐词查看、对照和批注，不影响六级学习进度。

以下步骤适用于已经准备好完整规范数据的词书：
