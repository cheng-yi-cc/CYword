...
安卓 0.1.0 提供 Android 7.0（API 24）及以上安装包。界面与排课逻辑复用 `src/`，通过 Capacitor 打包成本机应用。首版需联网获取词书、登录和同步；没有完整词书离线下载功能。2026-09-20 已完成生产 D1 授权、`learning_progress` 建表和官网同步函数部署，未认证请求返回 401；真实账号跨设备读写尚未验证。下述单词搜索、已学词进度、缓存与会话重构、凭据保护和同步修复属于当前源码，本轮尚未发布安装包或部署官网函数；正式 APK 需后续升级才具备这些行为。
...
以音记形与以熟带生复用同一套组件和审核数据，手机上点按参照词打开底部悬浮卡。默认本地预览可检查尚未发布的增强；正式 APK 仍读取线上词书，需要同时发布支持增强的客户端和词书数据。当前只完成浏览器窄屏验证，未做增强功能的安卓真机验收。
...
本轮已在 Windows 实际运行 Electron safeStorage/DPAPI，验证旧明文会话转密文、回读及退出清除；安卓原生插件的 Debug 构建通过。ADB 未连接真机，不能将构建成功视为 Keystore 真机迁移或真实账号跨设备同步已验收。

浏览器移动尺寸检查应覆盖首页、计划、学习、词根/长难句、回看、词汇掌握（筛选/搜索/重新评级）、单词搜索（候选/提交/详情返回）和复习；正式交付还需 APK 构建与签名校验。此前搜索改版通过 390×844 浏览器尺寸检查；本轮仍须在真机安装后确认键盘、安全区域、音频、覆盖升级和凭据迁移。
...
官网读取 `/downloads/android/latest.json`，稳定下载入口为 `/downloads/android/latest`；Windows 自动更新继续使用独立的 `/downloads/latest.yml`。本轮下载函数同时接受旧 v1 和新 v2 私有发布指针，公开 JSON 只提供官网路径、版本及校验信息。首次使用 v2 必须先部署兼容函数，再发布安装包。发布脚本在任何 R2 写入前，向固定官网域名的相应 `latest.json` 发起不跟随重定向的 HEAD 请求，只有响应头 `X-CYword-Release-Schemas` 确认支持 2 才继续；未发布 APK 的 404 响应也可声明此能力。普通 Git 推送不会部署官网函数。
