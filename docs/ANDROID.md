...
安卓 0.1.0 提供 Android 7.0（API 24）及以上安装包。界面与排课逻辑复用 `src/`，通过 Capacitor 打包成本机应用。首版需联网获取词书、登录和同步；没有完整词书离线下载功能。2026-09-20 已完成生产 D1 授权、`learning_progress` 建表和官网同步函数部署。线上未认证请求返回 401；本机旧令牌已过期，真实账号跨设备读写仍需重新登录后验证。
